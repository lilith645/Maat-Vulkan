...
impl InstanceV2 {
  pub fn new(app_name: String, app_version: u32, should_debug: bool) -> Arc<InstanceV2> {
    let function_pointers = OwnedOrRef::Ref(loader::auto_loader().unwrap());
    let entry_points = function_pointers.entry_points();
    
    let (vk, instance, layers) = InstanceV2::create_instanceV2(&entry_points, &function_pointers, app_name, app_version, should_debug);
  ...
  fn create_instanceV2(entry_points: &vk::EntryPoints, function_pointers: &OwnedOrRef<FunctionPointers<Box<dyn Loader + Sync + Send>>>, app_name: String, app_version: u32, should_debug: bool) -> (vk::InstancePointers, vk::Instance, Vec<CString>) {
    ...
    let appinfo = vk::ApplicationInfo {
      ...
      apiVersion: (1 as u32) << 22 | (0 as u32) << 12 | (0 as u32),
    };
    ...
    let instance: vk::Instance = unsafe {
      ...
      let instance_info = vk::InstanceCreateInfo {
        ...
        enabledExtensionCount: supported_extensions.len() as u32,
      };
    ...
    (vk_instance, instance, layer_names.to_vec())
  }
  ...
  pub fn get_extensions(&self) -> Vec<CString> {
    let function_pointers = OwnedOrRef::Ref(loader::auto_loader().unwrap());
    let entry_points = function_pointers.entry_points();
    supported_extensions(entry_points)
  }
